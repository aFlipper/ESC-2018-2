...
def removeComentario()
    ...
    for linha in codigo_sem_espaco
            if linha.find('') == 0
                codigo_sem_comentario.append(linha[0linha.find('')])
            elif linha.find('') != -1
                codigo_sem_comentario.append(linha[0linha.find('')] + 'n')
            else
...
def removeLabel()
    ...
    for linha in codigo_sem_comentario
        if linha.find('(') != -1
            labels[linha[1len(linha)-2]] = index - aux
            aux += 1
        else
    ...
    for linha in codigo_sem_labels
        for palavra in linha.split()
            if palavra[1len(palavra)] in labels.keys()
                codigo_sem_label_jump.append('@'+str(labels[palavra[1]])+'n')
            else
...
def removeVariavel()
    ...
    #Índice da memóriadicionário, começa em 16 pois R0 até R15 está reservado
    index = 16

    for linha in codigo_sem_label_jump
        if linha[01] == '@'
            if linha[1len(linha)-1] not in variaveis.keys() and not str(linha[1len(linha)-1]).isdigit()
                variaveis[linha[1len(linha)-1]] = index
                index += 1
    for linha in codigo_sem_label_jump
        if linha[1] == '@'
            if linha[1len(linha)-1] in variaveis.keys()
                codigo_sem_variavel.append('@'+str(variaveis[linha[1len(linha)-1]])+'n')
                #codigo_sem_variavel.append(bin(variaveis[linha[1len(linha)-1]])[2].zfill(16)+'n')
            else
                codigo_sem_variavel.append(linha)
                #codigo_sem_variavel.append(bin(int(linha[1len(linha)-1]))[2].zfill(16)+'n')
        else
            codigo_sem_variavel.append(linha)
...
def converteInstrucoesBinario()
    ...
    for linha in codigo_sem_variavel
        if linha.find('=') != -1
            if linha[linha.find(=)+1len(linha)-1] in instrucoes_comp_0
                a = 0
                codigo_instrucoes_bin.append('111' + str(a) +
                                             instrucoes_comp_0[linha[linha.find(=)+1len(linha)-1]] +
                ...
                a = 1
                codigo_instrucoes_bin.append('111' + str(a) +
                                             instrucoes_comp_1[linha[linha.find(=)+1len(linha)-1]] +
                                             instrucoes_dest[linha[linha.find(=)]] +
                                             '000' + 'n'
                                             )
        elif linha.find(';') != -1
            if linha[linha.find(;)] in instrucoes_comp_0
                ...
                codigo_instrucoes_bin.append('111' + str(a) +
                                             ...
                                             '000' +
                                             instrucoes_jmp[linha[linha.find(;)+1len(linha)-1]] + 'n'
                                             )
            else
                a = 1
                codigo_instrucoes_bin.append('111' + str(a) +
            ...
            codigo_instrucoes_bin.append(bin(int(linha[1]))[2].zfill(16) + 'n')
        else
